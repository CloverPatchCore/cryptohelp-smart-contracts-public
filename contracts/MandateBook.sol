...
contract MandateBook is AMandate, ReentrancyGuard {
    ...
    modifier onlyMandateOrAgreementOwner(uint256 mandateId) {
        ...
        require(
            sender == mandate.investor || sender == _agreements[mandate.agreement].manager,
    ...
    function getMandate(
        uint256 mandateId
    ) public view onlyExistMandate(mandateId) returns (AMandate.Mandate memory mandate) {
        return _mandates[mandateId];
    }

    function getMandateStatus(
        uint256 mandateId
    ) public view onlyExistMandate(mandateId) returns (AMandate.MandateLifeCycle) {
        return _mandates[mandateId].status;
    }

    function getAgreement(
        uint256 agreementId
    ) public view onlyExistAgreement(agreementId) returns (AMandate.Agreement memory agreement) {
        return _agreements[agreementId];
    }

    function getAgreementStatus(
        uint256 agreementId
    ) public view onlyExistAgreement(agreementId) returns (AMandate.AgreementLifeCycle) {
        return _agreements[agreementId].status;
    }

    function getAgreementPublishTimestamp(
        uint256 agreementId
    ) public view onlyExistAgreement(agreementId) returns(uint256) {
        return _agreements[agreementId].publishTimestamp;
    }

    function getAgreementCollateral(
        uint256 agreementId
    ) public view onlyExistAgreement(agreementId) returns(uint256) {
        return _agreements[agreementId].__collatAmount;
    }
    function getAgreementCommittedCapital(
        uint256 agreementId
    ) public view onlyExistAgreement(agreementId) returns(uint256) {
    ...
    function populateAgreement(
        ...
        if(agreement.__collatAmount > 0 && baseCoin != agreement.baseCoin) {
            require(agreement.baseCoin == baseCoin, "Can't change the basecoin when collateral deposited");
        }
        agreement.baseCoin = baseCoin;
        agreement.targetReturnRate = targetReturnRate;
        agreement.maxCollateralRateIfAvailable = maxCollateralRateIfAvailable;
    ...
    function depositCollateral(
        uint256 agreementId,
        uint256 amount
    ) external onlyExistAgreement(agreementId) onlyAgreementManager(agreementId) returns (uint256 transferred){
        Agreement storage agreement = _agreements[agreementId];
        require(address(0) != agreement.baseCoin, "baseCoin of agreement must not be equal to zero address");
        //if(msg.value > 0) processEthers();
        transferred = _transferDepositCollateral(agreementId, amount);
    ...
    function withdrawCollateral(
        uint256 agreementId,
        uint256 amount
    ) external onlyExistAgreement(agreementId) onlyAgreementManager(agreementId) onlyPositiveAmount(amount) {
        Agreement storage agreement = _agreements[agreementId];
    ...
    function _transferDepositCollateral(uint256 agreementId, uint256 amount) internal returns (uint256 transferred) {
        Agreement storage agreement = _agreements[agreementId];
    ...
    function __safeTransferFrom(
        ...
        transferredAmount = amount > allowance ? allowance : amount;
        ierc20.transferFrom(from, to, transferredAmount);
        uint256 bAfterFrom = ierc20.balanceOf(from);
        uint256 bAfterTo = ierc20.balanceOf(to);
        require(bBeforeTo.add(transferredAmount) == bAfterTo, "TransferFrom to-address balance mismatch");
        require(bBeforeFrom.sub(transferredAmount)== bAfterFrom, "TransferFrom to-address balance mismatch");
    }
    ...
    ) external onlyExistMandate(mandateId) returns (uint256 transferred) {
        Mandate storage mandate = _mandates[mandateId];
        uint256 agreementId = mandate.agreement;
        Agreement storage agreement = _agreements[agreementId];
        require(address(0) != agreement.baseCoin, "baseCoin of agreement must not be equal to zero address");
        require(agreement.status == AgreementLifeCycle.PUBLISHED, "Agreement status must be equal to PUBLISHED");
        transferred = _transferDepositCapital(agreement, mandate, amount, minCollatRateRequirement);
        emit DepositCapital(agreementId, agreement.manager, mandateId, mandate.investor, transferred);
    }

    function withdrawCapital(
        uint256 mandateId,
        uint256 amount
    ) external onlyExistMandate(mandateId) onlyMandateInvestor(mandateId) returns (uint256 transferred) {
        // @TODO: implement
        Mandate storage mandate = _mandates[mandateId];
        uint256 agreementId = mandate.agreement;
        Agreement storage agreement = _agreements[agreementId];
        ...
        emit WithdrawCapital(agreementId, agreement.manager, mandateId, mandate.investor, transferred);
        if(mandate.__committedCapital == 0) {
            _cancelEmptyMandate(agreement, mandate);
            emit CancelMandate(agreementId, agreement.manager, mandateId, mandate.investor);
        }
    }
    ...
    function _transferDepositCapital(
        ...
        /* the allowance on the chosen ERC20 coin has to be sufficient for the transferFrom, 
        taking into account all the previous allowances on possible earlier deals */
        /* UPD: still not convinced it's worth keeping the global allowances updated 
        require(IERC20(_agreements[id].baseCoin).allowance(msg.sender) >= 
        amount + _investors[msg.sender].globalAllowances[_agreements[id].baseCoin]); */
        transferred = __safeTransferFrom(agreement.baseCoin, msg.sender, address(this), amount);
    ...
    function cancelMandate(
        uint256 mandateId
    ) external onlyExistMandate(mandateId) onlyMandateOrAgreementOwner(mandateId) {
        // @TODO validations
        Mandate storage mandate = _mandates[mandateId];
        uint256 agreementId = mandate.agreement;
        address investor = mandate.investor;
        Agreement storage agreement = _agreements[agreementId];
        address manager = agreement.manager;
        uint256 transferred = _transferWithdrawCapital(agreement, mandate, mandate.__committedCapital);
        emit WithdrawCapital(agreementId, manager, mandateId, investor, transferred);
        _cancelEmptyMandate(agreement, mandate);
        emit CancelMandate(agreementId, manager, mandateId, investor);
    }
    ...
    function cancelAgreement(
        uint256 agreementId
    ) external view onlyExistAgreement(agreementId) onlyAgreementManager(agreementId) {
        revert("to be implemented later");
    }

    function unpublishAgreement(
        uint256 agreementId
    ) external onlyExistAgreement(agreementId) onlyAgreementManager(agreementId) {
        // @TODO implement
        Agreement storage agreement = _agreements[agreementId];
    ...
    function commitToAgreement(
        ...
        uint256 mandateId = _mandates.length;
        _mandates.push();
        Mandate storage mandate = _mandates[mandateId];
        mandate.status = MandateLifeCycle.COMMITTED;
        mandate.investor = investor;
        mandate.agreement = agreementId;
        transferred = _transferDepositCapital(agreement, mandate, amount, minCollatRateRequirement);
    ...
    function setExpiredAgreement(uint256 agreementId) public {
        ...
        require(!agreementClosed[agreementId], "Agreement trades are not closed");
    ...
    function settleMandate(uint256 mandateId) public onlyMandateOrAgreementOwner(mandateId) nonReentrant {
        ...
        require(agreementClosed[agreementId], "Agreement not closed");
        //find the share of the mandate in the pool and multiply by the finalBalance
        uint256 finalAgreementTradeBalance = balances[agreementId];
        // the final trade balance per this mandate is calculated as a share in the entire trade balance
        uint256 mandateFinalTradeBalance = mandate.__committedCapital.mul(
            finalAgreementTradeBalance).div(agreement.__committedCapital);
        ...
        if (mandateFinalTradeBalance < profitAbsTarget) {
            //calculate the ideal compensation from the collateral to cover the gap between real and target profits
    ...
    function withdrawManagerCollateral(
        ...
        uint256 finalAgreementTradeBalance = balances[agreementId].add(agreement.__collatAmount);
